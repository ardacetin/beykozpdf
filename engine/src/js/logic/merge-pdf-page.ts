...
export async function merge() {
  ...
  try {
    ...
    if (jobs.length === 0) {
      showAlert('Error', 'Birleştirilecek dosya veya sayfa seçilmedi.');
      hideLoader();
      return;
    }
...
export async function refreshMergeUI() {
  ...
  showLoader('PDF belgeleri hazırlanıyor…');
  try {
    mergeState.pdfDocs = {};
    mergeState.pdfBytes = {};
    ...
    showAlert('Error', 'PDF dosyalarından biri açılamadı.');
    return;
  } finally {
    hideLoader();
  }
  ...
  (state.files as File[]).forEach((f, index) => {
    ...
    label.textContent = `Sayfalar (ör. 1-3, 5) · Toplam: ${pageCount}`;
    ...
    input.placeholder = 'Tüm sayfalar için boş bırakın';
    ...
    deleteBtn.title = 'Dosyayı kaldır';
    deleteBtn.onclick = (e) => {
      e.stopPropagation();
  ...
  // PDF bytes and the sortable page/file list must be ready before merging.
